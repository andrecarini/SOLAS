...
# Summary modes
SUMMARY_MODES = ['greedy', 'sampled', 'hybrid']
...
# Best parameters per model (for the "Best Params Comparison" experiment)
# These represent what we consider optimal settings for each model
...
def generate_experiments() -> List[Dict[str, Any]]:
    ...
    # EXPERIMENT 4: Summary Mode Impact (FIX: Specify LLM = phi-3-mini)
    for summary_mode in SUMMARY_MODES:
        experiments.append({
            'id': f'summary_mode_{summary_mode}',
            'experiment_type': 'summary_mode',
            'description': f'Summary mode: {summary_mode} (phi-3-mini)',
            'stages': ['translation', 'summary', 'podcast'],
            'config': {**BASELINE, 'summary_mode': summary_mode},
        })
    ...
    for chunk_size in [2000, 4000]:
        experiments.append({
            ...
            'description': f'Chunk size: {chunk_size} chars',
            'stages': ['translation', 'summary', 'podcast'],
            'config': {**BASELINE, 'chunk_size_chars': chunk_size},
        })
    ...
    # EXPERIMENT 7: Best Params Comparison (compare 4 LLMs with their optimal settings)
    for llm_model in LLM_MODELS:
        best_params = BEST_PARAMS_PER_MODEL[llm_model]
        experiments.append({
            'id': f'best_{llm_model.split("/")[-1]}',
            'experiment_type': 'best_params',
            'description': f'Best params: {llm_model.split("/")[-1]}',
            'stages': ['translation', 'summary', 'podcast'],
            'config': {
                **BASELINE,
