...
from .evaluation_utils import (
    ...
    config_hash, is_config_cached,
    unload_asr, unload_all_models
)
...
def run_evaluation(
    ...
    for exp in experiments:
        if is_experiment_complete_fn(results, exp['id']):
            skipped_complete += 1
            continue
        cached_id = is_config_cached(results, exp['config'], exp['stages'], hardware_info)
        if cached_id:
            log_fn(f"Skipping {exp['id']} (duplicate config, see {cached_id})", 'detail')
            skipped_duplicate += 1
            continue
    ...
    log_fn(f"Skipped (duplicate config): {skipped_duplicate}", 'info')
        ...
        try:
            ...
            if stages == ['asr']:
                ...
                if 'translation' in stages:
                    ...
                    save_experiment_output_fn(exp_id, 'translation', 'output', trans_result['output']['translated_text'])
                ...
                if 'summary' in stages:
                    ...
                    save_experiment_output_fn(exp_id, 'summary', 'output', sum_result['output']['summary'])
