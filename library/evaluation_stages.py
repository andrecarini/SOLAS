...
def run_summary_stage(
    ...
    Implements the CORRECT solas_pipeline.py logic:
    ...
    # FIX: Configure chunk generation correctly based on summary_mode
    # Matches solas_pipeline.py:893-912
    if summary_mode == 'sampled':
        chunk_do_sample = True
        chunk_temperature = 0.2  # CORRECT: 0.2, not 0.7!
        chunk_top_p = 0.9
    else:  # greedy or hybrid - chunks use same settings
        chunk_do_sample = False
        chunk_temperature = None
        chunk_top_p = None

    # FIX: Configure aggregation generation based on summary_mode
    # Matches solas_pipeline.py:915-925
    if summary_mode == 'hybrid':
        agg_do_sample = False  # Hybrid uses greedy aggregation
        agg_temperature = None
        agg_top_p = None
    else:
        agg_do_sample = chunk_do_sample  # Same as chunks
    ...
    with metrics_collector_cls('summary') as collector:
        ...
        for chunk in tqdm(chunks, desc="Summarizing", unit="chunk", leave=False):
            ...
            if chunk_temperature is not None:
                gen_kwargs['temperature'] = chunk_temperature
            if chunk_top_p is not None:
                gen_kwargs['top_p'] = chunk_top_p
            if repetition_penalty is not None:
                gen_kwargs['repetition_penalty'] = repetition_penalty
            ...
            # FIX: Normalize bullets (matches solas_pipeline.py:968-978)
            norm_lines = []
        ...
        # FIX: PHASE 2: Aggregate partial summaries (THIS WAS COMPLETELY MISSING!)
        # Matches solas_pipeline.py:989-1040
        aggregation_prompt = (
        ...
        if agg_temperature is not None:
            agg_kwargs['temperature'] = agg_temperature
        if agg_top_p is not None:
            agg_kwargs['top_p'] = agg_top_p
        if repetition_penalty is not None:
            agg_kwargs['repetition_penalty'] = repetition_penalty

        with torch.no_grad():
            output_ids = model.generate(input_ids=input_ids, attention_mask=attention_mask, **agg_kwargs)

        gen_only = output_ids[0, input_ids.shape[1]:]
        summary = tokenizer.decode(gen_only, skip_special_tokens=True).strip()

        # FIX: Normalize final bullets (matches solas_pipeline.py:1029-1038)
        final_lines = []
...
def run_podcast_stage(
    ...
    Matches solas_pipeline.py generate_podcast_script implementation.
    ...
    # Note: podcast_creativity_temp is stored but not used - podcast uses do_sample=False
    # This matches solas_pipeline.py which also doesn't use temperature for podcast
    ...
    # Generation kwargs - do_sample=False matches solas_pipeline.py
    gen_kwargs = {
        'max_new_tokens': max_new_tokens,
        'do_sample': False,
        'pad_token_id': tokenizer.eos_token_id,
    ...
    with metrics_collector_cls('podcast') as collector:
        ...
        for i, chunk in pbar:
            ...
            # Enforce label format for this segment (matching solas_pipeline.py)
